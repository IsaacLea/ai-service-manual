 ...
 * *UPDATE* the pdf parsing has been moved to server side and is undergoing testing
 * 
...
export default function Upload() {
  ...
  const [indexDisplayName, setIndexDisplayName] = useState<string>("");
  ...
  return (
    <div className="grid justify-items-center h-full bg-gray-100 w-full">
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded shadow-md w-full min-h-[32rem]">
        ...
        <label className="mb-4 block">
          <span className="block text-gray-700">Index display name</span>
          <input
            type="text"
            value={indexDisplayName}
            onChange={(e) => setIndexDisplayName(e.target.value)}
            className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
            placeholder="Enter index display name"
            required
          />
        </label>
